    ...
    try:
        ...
        if app_connector:
            app_connector.debug_print(
                "Unable to save state file. Error: {}".format(str(e))
            )
        return phantom.APP_ERROR
...
def _remove_app_state(asset_id, app_connector=None):
    ...
    try:
        ...
        if app_connector:
            app_connector.debug_print(
                "Unable to remove state file. Error: {}".format(str(e))
            )
        return phantom.APP_ERROR
...
def handle_request(request, path_parts):
    ...
    try:
        ...
        return _return_response(
            asset_id, "Error handling request: {}".format(str(e)), 400
        )
...
class MsGraphForSharepointConnector(BaseConnector):
    ...
    def _validate_integer(self, action_result, parameter, key, allow_zero=True):
        ...
        if parameter is not None:
            try:
                if not float(parameter).is_integer():
                    return (
                        action_result.set_status(
                            phantom.APP_ERROR,
                            MS_SHAREPOINT_VALID_INTEGER_MSG.format(key),
                        ),
                        None,
                    )
                ...
                return (
                    action_result.set_status(
                        phantom.APP_ERROR, MS_SHAREPOINT_VALID_INTEGER_MSG.format(key)
                    ),
                    None,
                )
            ...
            if parameter < 0:
                return (
                    action_result.set_status(
                        phantom.APP_ERROR,
                        MS_SHAREPOINT_NON_NEGATIVE_INTEGER_MSG.format(key),
                    ),
                    None,
                )
            ...
            if not allow_zero and parameter == 0:
                return (
                    action_result.set_status(
                        phantom.APP_ERROR,
                        MS_SHAREPOINT_POSITIVE_INTEGER_MSG.format(key),
                    ),
                    None,
                )
    ...
    def _make_rest_call_helper(
        ...
        if msg and any(
            failure_message in msg for failure_message in MS_AUTH_FAILURE_MSGS
        ):
    ...
    def _make_rest_calls_to_phantom(self, url, action_result, verify=False):
        ...
        if not r:
            ...
            if r.text:
                message = "{} Error from Server: {}".format(
                    message, r.text.replace("{", "{{").replace("}", "}}")
                )
            return (
                action_result.set_status(
                    phantom.APP_ERROR,
                    "Error retrieving system info, {0}".format(message),
                ),
                None,
            )
        ...
        try:
            ...
            return (
                action_result.set_status(
                    phantom.APP_ERROR,
                    "Error processing response JSON. Error: {}".format(error_message),
                ),
                None,
            )
    ...
    def _get_asset_name(self, action_result):
        ...
        rest_endpoint = SOAR_ASSET_INFO_URL.format(
            url=self.get_phantom_base_url(), asset_id=asset_id
        )
        ret_val, resp_json = self._make_rest_calls_to_phantom(
            rest_endpoint, action_result
        )
        if phantom.is_fail(ret_val):
            return ret_val, None
    ...
    def _get_phantom_base_url(self, action_result):
        ret_val, resp_json = self._make_rest_calls_to_phantom(
            SOAR_SYS_INFO_URL.format(url=self.get_phantom_base_url()), action_result
        )
        if phantom.is_fail(ret_val):
            return ret_val, None
        ...
        if not phantom_base_url:
            return (
                action_result.set_status(
                    phantom.APP_ERROR,
                    "Phantom Base URL not found in System Settings. Please specify this value in System Settings",
                ),
                None,
            )
        ...
        while True:
            if next_link:
                ret_val, response = self._make_rest_call_helper(
                    endpoint, action_result, next_link=next_link
                )
            else:
                ret_val, response = self._make_rest_call_helper(
    ...
    def _handle_add_item(self, param):
        ...
        endpoint = "{}/items".format(
            MS_GET_LIST_ENDPOINT.format(
                self._site_id, urllib.parse.quote(param[MS_SHAREPOINT_JSON_LIST])
            )
        )
        ret_val, item = self._make_rest_call_helper(
            method="post",
            endpoint=endpoint,
    ...
    def _handle_list_lists(self, param):
        ...
        if not self._site_id:
            return action_result.set_status(
                phantom.APP_ERROR,
                MS_SHAREPOINT_ERROR_MISSING_SITE_ID.format(
                    "retrieving lists information"
                ),
            )
        ...
        ret_val, lists = self._paginator(
            action_result, MS_LIST_LISTS_ENDPOINT.format(self._site_id), limit=limit
        )
        if phantom.is_fail(ret_val):
            return action_result.get_status()
    ...
    def _handle_get_list(self, param):
        ...
        if not self._site_id:
            return action_result.set_status(
                phantom.APP_ERROR,
                MS_SHAREPOINT_ERROR_MISSING_SITE_ID.format("retrieving a list"),
            )
        ...
        endpoint = MS_GET_LIST_ENDPOINT.format(
            self._site_id, urllib.parse.quote(param[MS_SHAREPOINT_JSON_LIST])
        )
        ...
        ret_val, list_items = self._paginator(
            action_result, endpoint, params=params, limit=limit
        )
        if phantom.is_fail(ret_val):
            return action_result.get_status()
    ...
    def _handle_get_file(self, param):
        ...
        if not self._site_id:
            return action_result.set_status(
                phantom.APP_ERROR,
                MS_SHAREPOINT_ERROR_MISSING_SITE_ID.format("retrieving a file"),
            )

        sp_path = param[MS_SHAREPOINT_JSON_FILE_PATH].strip("/")
        sp_file = param[MS_SHAREPOINT_JSON_FILE_NAME]
        endpoint = MS_GET_FILE_METADATA_ENDPOINT.format(
            self._site_id, urllib.parse.quote(sp_path), urllib.parse.quote(sp_file)
        )
        ...
        ret_val, tmp_file_path = self._make_rest_call_helper(
            MS_GET_FILE_CONTENT_ENDPOINT.format(endpoint),
            action_result,
            download=True,
        )
        if phantom.is_fail(ret_val):
    ...
    def _handle_remove_file(self, param):
        ...
        if not self._site_id:
            return action_result.set_status(
                phantom.APP_ERROR,
                MS_SHAREPOINT_ERROR_MISSING_SITE_ID.format("removing a file"),
            )
        ...
        ret_val, _ = self._make_rest_call_helper(
            endpoint, action_result, method="delete"
        )
        if phantom.is_fail(ret_val):
            return action_result.get_status()

        return action_result.set_status(
            phantom.APP_SUCCESS, "Successfully deleted file"
        )
    ...
    def initialize(self):
        ...
        self._endpoint_test_connectivity = config.get(
            "endpoint_test_connectivity", MS_TEST_CONNECTIVITY_ENDPOINT
        )
        self._access_token = self._state.get(MS_SHAREPOINT_JSON_TOKEN, {}).get(
            MS_SHAREPOINT_JSON_ACCESS_TOKEN, None
        )
    ...
    def finalize(self):
        try:
            if self._state.get(MS_SHAREPOINT_JSON_TOKEN, {}).get(
                MS_SHAREPOINT_JSON_ACCESS_TOKEN
            ):
                self._state[MS_SHAREPOINT_JSON_TOKEN][
                    MS_SHAREPOINT_JSON_ACCESS_TOKEN
    ...
    if username and password:
        try:
            login_url = "{}/login".format(
                MsGraphForSharepointConnector._get_phantom_base_url()
            )
            ...
            print(
                "Unable to get session id from the platform. Error: {}".format(str(e))
            )
            sys.exit(1)
